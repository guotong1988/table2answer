...
def load_concat_wemb(fn1, fn2,use_small=False):
    wemb1 = load_word_emb(fn1,use_small=use_small)
    wemb2 = load_para_wemb(fn2)
...
def load_word_and_type_emb(fn1, fn2, sql_data, table_data, db_content, is_list=False, use_htype=False,use_small = False):
    word_to_idx = {'<UNK>':0, '<BEG>':1, '<END>':2}
    word_num = 3
    N_word = 300
    embs = [np.zeros(N_word, dtype=np.float32) for _ in range(word_num)]
    _, _, word_emb = load_concat_wemb(fn1, fn2, use_small)
